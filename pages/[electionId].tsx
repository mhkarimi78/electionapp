    ...
    <div className="bg-slate-300 h-screen">
      <div className=" flex justify-around p-20">
        <div className="text-black flex flex-col items-center">
          <p className="my-10">ElectionId {Number(id)}</p>
          {isElectionLoading ? (
            "loading Election Details"
          ) : (
            <div className="flex flex-col items-center">
              <p>Election Id: {election?.id._hex}</p>
              <p className="my-10">Election Name: {election?.name}</p>
              <p>Election Start Date: {startDate}</p>
              <p className="my-10">Election End Date: {endDate}</p>
              <p>
                Election MinPercentage to Win: {Number(election?.minPercentage._hex)}
              </p>
              <p className="my-10">
                Election Number of Winners: {election?.numWinners._hex}
              </p>
